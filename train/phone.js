...
function phone(strng, num) {
	let reg = new RegExp(`\n?(.*${num}.*)\n?`, 'g');
	let line = strng.match(reg);
	if (!line) return `Error => Not found: ${num}`;
	if (line.length > 1) return `Error => Too many people: ${num}`;
	line = line[0];
	let name = line.match(/<(.+){1,2}>/);
	let phoneNumber = line.match(/[^ ]?\+(\d+-\d+-\d+-\d+)/);
	let address = line.replace(name[0], '').replace(phoneNumber[0], '').replace(/[;_,!\?]/g,' ').replace(/\ {2,}/g, ' ').trim();
	return `Phone => ${phoneNumber[1]}, Name => ${name[1]}, Address => ${address}\n\t`;
}
